...
function treeBoxes(urlService, jsonData)
{
	...
	var rectNode = { width : 300, height : 60, textMargin : 5 },
	...
	{
		tree = d3.layout.tree().size([ 600, width ]);
		root = jsonData;
		root.fixed = true;
		...
		console.log("The nodes ", nodeDict);
	}
...
function collapse(d) {
	...
	function dfs(startNode) {
		nodeDict[startNode.code] = [];
		if (startNode.children == null){
			return
		}

		for(let i = 0; i < startNode.children.length; i++) {
			let childNode = dfs(startNode.children[i]);
			nodeDict[startNode.code].push(childNode);
		}
